...
export function Step5BehaviorSafety({
  ...
  // Conversational Acoustics State
  const conversationalFillersEnabled = agentData.runtime?.conversational_fillers_enabled ?? true;
  const fillerPhrases = agentData.runtime?.filler_phrases ?? [
    "Got it, let me check that for you...",
    "Understood, give me one moment...",
    "Sure thing, looking into that right now...",
    "Let me see..."
  ];
  const [newFillerPhrase, setNewFillerPhrase] = useState("");
  const [showAddFiller, setShowAddFiller] = useState(false);

  const endpointingMode = agentData.listen?.endpointing_mode ?? "adaptive";
  const currentEndpointing = agentData.listen?.endpointing ?? 500;

  const backchannelingEnabled = agentData.runtime?.backchanneling_enabled ?? true;
  const backchannelInterval = agentData.runtime?.backchannel_interval_seconds ?? 4.5;
  const backchannelPhrases = agentData.runtime?.backchannel_phrases ?? [
    "Mhm",
    "Right",
    "I understand",
    "Yeah"
  ];
  const [newBackchannelCue, setNewBackchannelCue] = useState("");
  const [showAddBackchannel, setShowAddBackchannel] = useState(false);

  const toggleConversationalFillers = () => {
    setAgentData((prev) => ({
      ...prev,
      runtime: {
        ...prev.runtime!,
        conversational_fillers_enabled: !conversationalFillersEnabled
      }
    }));
  };

  const handleAddFillerPhrase = () => {
    if (!newFillerPhrase.trim()) return;
    const updated = [...fillerPhrases, newFillerPhrase.trim()];
    setAgentData((prev) => ({
      ...prev,
      runtime: {
        ...prev.runtime!,
        filler_phrases: updated
      }
    }));
    setNewFillerPhrase("");
    setShowAddFiller(false);
  };

  const handleRemoveFillerPhrase = (index: number) => {
    const updated = fillerPhrases.filter((_, i) => i !== index);
    setAgentData((prev) => ({
      ...prev,
      runtime: {
        ...prev.runtime!,
  ...
  const handleSelectEndpointingMode = (mode: "rapid" | "balanced" | "dictation" | "adaptive") => {
    let epVal = 500;
    if (mode === "rapid") epVal = 400;
    else if (mode === "dictation") epVal = 850;
    else if (mode === "adaptive") epVal = 750;

    setAgentData((prev) => ({
      ...prev,
      listen: {
        ...(prev.listen || { provider: "deepgram", model: "nova-3", language: "en" }),
  ...
  const handleSetBackchannelInterval = (sec: number) => {
    setAgentData((prev) => ({
      ...prev,
      runtime: {
        ...prev.runtime!,
        backchannel_interval_seconds: sec
      }
    }));
  };

  const handleAddBackchannelCue = () => {
    if (!newBackchannelCue.trim()) return;
    const updated = [...backchannelPhrases, newBackchannelCue.trim()];
    setAgentData((prev) => ({
      ...prev,
      runtime: {
        ...prev.runtime!,
        backchannel_phrases: updated
      }
    }));
    setNewBackchannelCue("");
    setShowAddBackchannel(false);
  };

  const handleRemoveBackchannelCue = (index: number) => {
    const updated = backchannelPhrases.filter((_, i) => i !== index);
    setAgentData((prev) => ({
      ...prev,
      runtime: {
        ...prev.runtime!,
  ...
  return (
    <div className="space-y-6 text-left">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h4 className="text-xs font-bold text-[var(--color-heading)]">
                  Allow Customer Interruptions
                </h4>
                <InfoTooltip
                  content="When enabled, the AI immediately stops speaking (barge-in) as soon as the caller starts talking."
                  position="top"
                />
              </div>
      ...
      {/* SECTION B: Conversational Realism & Spoken Acoustics */}
      <div className="space-y-4">
        <div className="flex items-center gap-1.5 pb-1">
          <Sparkles className="w-4 h-4 text-[var(--color-primary)]" />
          <h3 className="text-xs font-bold text-[var(--color-heading)] uppercase tracking-wider flex items-center gap-1.5">
            <span>Conversational Realism &amp; Spoken Acoustics</span>
            <InfoTooltip
              content="Configure natural human conversational acoustics, fillers during complex thinking, adaptive turn-taking for dictation, and continuous monologue backchanneling."
              position="top"
            />
          </h3>
          <Badge variant="primary" size="sm" className="text-[10px]">
        ...
        {/* Acoustics Grid */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
          {/* Card 1: Conversational Fillers & Natural Thinking Sounds (6 cols) */}
          <div className="lg:col-span-6 p-4 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-[var(--radius-main,0.5rem)] shadow-2xs space-y-3 flex flex-col justify-between">
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-[var(--color-heading)] flex items-center gap-1.5">
                      <span>Conversational Thinking Fillers</span>
                      <InfoTooltip
                        content="When formulating answers, computing dates, or looking up customer data, the AI speaks natural thinking sounds ('Got it, let me check...', 'Understood, one moment...') rather than leaving dead silence."
                        position="top"
                      />
                    </h4>
                  </div>
                ...
                <div className="flex items-center gap-2">
                  <span className={`text-[11px] font-bold font-mono ${conversationalFillersEnabled ? "text-emerald-600 dark:text-emerald-400" : "text-[var(--color-muted)]"}`}>
                    {conversationalFillersEnabled ? "Active" : "Off"}
                  </span>
                  <button
                    type="button"
                    role="switch"
                    aria-checked={conversationalFillersEnabled}
                    onClick={toggleConversationalFillers}
                    className={`w-9 h-5 flex items-center rounded-full p-0.5 transition-colors cursor-pointer ${
                      conversationalFillersEnabled ? "bg-[var(--color-primary)]" : "bg-[var(--color-border)]"
                    }`}
                  >
                    <div
                      className={`bg-white w-4 h-4 rounded-full shadow transform transition-transform ${
                        conversationalFillersEnabled ? "translate-x-4" : "translate-x-0"
                      }`}
                    />
                  </button>
                </div>
              ...
              <p className="text-[11px] text-[var(--color-muted)]">
                Emits natural verbal acknowledgments during multi-second processing turns to eliminate dead air.
              </p>

              {conversationalFillersEnabled && (
                <div className="space-y-2 pt-1">
                  <div className="flex flex-wrap gap-1.5">
                    {fillerPhrases.map((phrase, idx) => (
                      <span
                        key={idx}
                        className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[11px] bg-[var(--color-surface-muted)] border border-[var(--color-border)] text-[var(--color-heading)] font-medium"
                      >
                        <span>"{phrase}"</span>
                        {fillerPhrases.length > 1 && (
                          <button
                            type="button"
                            onClick={() => handleRemoveFillerPhrase(idx)}
                            className="text-[var(--color-muted)] hover:text-red-500 cursor-pointer ml-0.5"
                          >
                            <X className="w-3 h-3" />
                          </button>
                        )}
                  ...
                  {showAddFiller ? (
                    <div className="flex items-center gap-1.5 pt-1">
                      <input
                        type="text"
                        placeholder="e.g. Let me see..."
                        value={newFillerPhrase}
                        onChange={(e) => setNewFillerPhrase(e.target.value)}
                        onKeyDown={(e) => e.key === "Enter" && handleAddFillerPhrase()}
                        className="w-full h-7 px-2 text-xs bg-[var(--color-surface-muted)] border border-[var(--color-border)] rounded text-[var(--color-heading)] focus:outline-none focus:border-[var(--color-primary)]"
                        autoFocus
                      />
                      <Button size="sm" variant="primary" onClick={handleAddFillerPhrase} className="h-7 px-2.5 text-xs shrink-0 cursor-pointer">
                        Add
                      </Button>
                      <button
                        type="button"
                        onClick={() => { setShowAddFiller(false); setNewFillerPhrase(""); }}
                        className="text-xs text-[var(--color-muted)] hover:text-[var(--color-heading)] px-1 cursor-pointer"
                      >
                        Cancel
                      </button>
                    </div>
                    ...
                    <button
                      type="button"
                      onClick={() => setShowAddFiller(true)}
                      className="text-[11px] font-semibold text-[var(--color-primary)] hover:underline flex items-center gap-1 pt-0.5 cursor-pointer"
                    >
                      <Plus className="w-3 h-3" />
                      <span>+ Add custom thinking phrase</span>
                    </button>
                  )}
                </div>
              )}
          ...
          {/* Card 2: Continuous Monologue Backchanneling (6 cols) */}
          <div className="lg:col-span-6 p-4 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-[var(--radius-main,0.5rem)] shadow-2xs space-y-3 flex flex-col justify-between">
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-[var(--color-heading)] flex items-center gap-1.5">
                      <span>Monologue Backchanneling</span>
                      <InfoTooltip
                        content="Signals active human listening with subtle verbal cues ('Mhm', 'Right', 'Yeah') when the caller speaks an extended monologue without pausing."
                        position="top"
                      />
                    </h4>
                  </div>
                ...
                <div className="flex items-center gap-2">
                  <span className={`text-[11px] font-bold font-mono ${backchannelingEnabled ? "text-emerald-600 dark:text-emerald-400" : "text-[var(--color-muted)]"}`}>
                    {backchannelingEnabled ? "Active" : "Off"}
                  </span>
                  <button
                    type="button"
                    role="switch"
                    aria-checked={backchannelingEnabled}
                    onClick={toggleBackchanneling}
                    className={`w-9 h-5 flex items-center rounded-full p-0.5 transition-colors cursor-pointer ${
                      backchannelingEnabled ? "bg-[var(--color-primary)]" : "bg-[var(--color-border)]"
                    }`}
                  >
                    <div
                      className={`bg-white w-4 h-4 rounded-full shadow transform transition-transform ${
                        backchannelingEnabled ? "translate-x-4" : "translate-x-0"
                      }`}
                    />
                  </button>
                </div>
              ...
              <p className="text-[11px] text-[var(--color-muted)]">
                Emits subtle non-interruptive acknowledgments so callers know the AI is listening closely.
              </p>
              ...
              {backchannelingEnabled && (
                <div className="space-y-2.5 pt-1">
                  <div>
                    ...
                    <div className="flex items-center gap-1.5 pt-1">
                      {[
                        { sec: 3.5, label: "3.5s (Frequent)" },
                        { sec: 4.5, label: "4.5s (Natural)" },
                        { sec: 6.0, label: "6.0s (Relaxed)" }
                      ].map((item) => (
                        <button
                          key={item.sec}
                          type="button"
                          onClick={() => handleSetBackchannelInterval(item.sec)}
                          className={`px-2.5 py-1 text-[11px] rounded-[var(--radius-main,0.25rem)] border transition-all cursor-pointer font-medium ${
                            backchannelInterval === item.sec
                              ? "bg-[var(--color-primary)] text-white border-[var(--color-primary)] font-bold shadow-2xs"
                              : "bg-[var(--color-surface-muted)] text-[var(--color-heading)] border-[var(--color-border)] hover:border-[var(--color-border-strong,var(--color-border))]"
                          }`}
                        >
                  ...
                  <div className="flex flex-wrap gap-1.5 pt-1">
                    {backchannelPhrases.map((cue, idx) => (
                      <span
                        key={idx}
                        className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[11px] bg-[var(--color-surface-muted)] border border-[var(--color-border)] text-[var(--color-heading)] font-mono font-semibold"
                      >
                        <span>"{cue}"</span>
                        {backchannelPhrases.length > 1 && (
                          <button
                            type="button"
                            onClick={() => handleRemoveBackchannelCue(idx)}
                            className="text-[var(--color-muted)] hover:text-red-500 cursor-pointer ml-0.5"
                          >
                            <X className="w-3 h-3" />
                          </button>
                  ...
                  {showAddBackchannel ? (
                    <div className="flex items-center gap-1.5 pt-1">
                      <input
                        ...
                        value={newBackchannelCue}
                        onChange={(e) => setNewBackchannelCue(e.target.value)}
                        onKeyDown={(e) => e.key === "Enter" && handleAddBackchannelCue()}
                        className="w-full h-7 px-2 text-xs bg-[var(--color-surface-muted)] border border-[var(--color-border)] rounded text-[var(--color-heading)] focus:outline-none focus:border-[var(--color-primary)] font-mono"
                        autoFocus
                      />
                      <Button size="sm" variant="primary" onClick={handleAddBackchannelCue} className="h-7 px-2.5 text-xs shrink-0 cursor-pointer">
                        Add
                      </Button>
                      <button
                        type="button"
                        onClick={() => { setShowAddBackchannel(false); setNewBackchannelCue(""); }}
                        className="text-xs text-[var(--color-muted)] hover:text-[var(--color-heading)] px-1 cursor-pointer"
                      >
                        Cancel
                      </button>
          ...
          {/* Card 3: Adaptive Turn-Taking & Endpointing (Full Width 12 cols) */}
          <div className="lg:col-span-12 p-4 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-[var(--radius-main,0.5rem)] shadow-2xs space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h4 className="text-xs font-bold text-[var(--color-heading)] flex items-center gap-1.5">
                    <span>Adaptive Turn-Taking &amp; Endpointing</span>
                    <InfoTooltip
                      content="Controls how long the AI waits for the caller to finish speaking before responding. Adaptive mode extends pause tolerance for emails, phone numbers, and addresses so callers are never cut off."
                      position="top"
                    />
                  </h4>
                  <p className="text-[11px] text-[var(--color-muted)] mt-0.5">
                    Automatically accommodates natural human hesitation pauses when dictating numbers, names, and postal codes.
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 self-start sm:self-auto">
                <span className="text-[11px] font-mono text-[var(--color-muted)]">
                  Buffer: <strong className="text-[var(--color-heading)]">{currentEndpointing}ms</strong>
                </span>
              ...
              {[
                {
                  ...
                  badge: "Recommended",
                  desc: "Automatically expands buffer (750-850ms) for phone/email dictation; keeps standard Q&A brisk (450ms)."
                },
                {
                  mode: "balanced" as const,
                  title: "Balanced (500ms)",
                  badge: "Standard",
                  desc: "Balanced turn-taking for general inquiries, customer support, and standard multi-clause speech."
                },
                {
                  mode: "dictation" as const,
                  title: "Dictation & Numbers (850ms)",
                  badge: "High Tolerance",
                  desc: "Generous pause tolerance designed specifically for callers reciting numbers, credit cards, or addresses."
                },
                {
                  mode: "rapid" as const,
                  title: "Rapid Q&A (400ms)",
                  badge: "Ultra-Fast",
                  desc: "High-cadence rapid back-and-forth for short single-phrase confirmations and quick surveys."
                }
              ].map((item) => (
                <div
                  key={item.mode}
                  onClick={() => handleSelectEndpointingMode(item.mode)}
                  className={`p-3 rounded-[var(--radius-main,0.375rem)] border text-left cursor-pointer transition-all ${
                    endpointingMode === item.mode
                      ? "border-[var(--color-primary)] bg-[var(--color-primary)]/5 ring-1 ring-[var(--color-primary)] shadow-2xs"
                      : "border-[var(--color-border)] bg-[var(--color-surface-muted)]/50 hover:border-[var(--color-border-strong,var(--color-border))] hover:bg-[var(--color-surface-muted)]"
                  }`}
                >
                  <div className="flex items-center justify-between gap-1 mb-1">
                    <span className="text-xs font-bold text-[var(--color-heading)]">{item.title}</span>
      ...
      {/* SECTION C: Safety Guardrails (Full Width) */}
